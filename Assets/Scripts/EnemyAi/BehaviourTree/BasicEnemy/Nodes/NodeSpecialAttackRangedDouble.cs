...
public class NodeSpecialAttackRangedDouble : IBehaviourTreeNode
{
    ...
    float delayBetweenSecondShot = 0.1f;
    bool attacking;
    bool projectileLaunched;
    bool projectileLaunched2;
    ...
    public NodeSpecialAttackRangedDouble(EnemyAi _enemy)
    {
        enemy = _enemy;
        ...
        if (attacking)
        {
            ...
            else if (timer >= frontSwing && timer < frontSwing + attackTime)        //Beginning of the attack after front swing. Performs attack
            {
                ...
                if (!projectileLaunched)
                {
                    ...
                    Vector3 tempRight = enemy.target.GetChild(0).position + enemy.target.right;
                    GameObject rangedProjectile = enemy.ShootProjectile(tempRight - tempPos, projectileSpeed);
                    rangedProjectile.GetComponent<EnemyProjectile>().knockback = enemy.specialKnockback;
                    projectileLaunched = true;
                }
                if (timer >= frontSwing + delayBetweenSecondShot && !projectileLaunched2)
                {
                    Vector3 tempPos = new Vector3(enemy.transform.position.x, enemy.transform.position.y + (enemy.transform.lossyScale.y * 0.5f), enemy.transform.position.z) + enemy.transform.forward * 0.5f;
                    Vector3 tempLeft = enemy.target.GetChild(0).position - enemy.target.right;
                    GameObject rangedProjectile2 = enemy.ShootProjectile(tempLeft - tempPos, projectileSpeed);
                    rangedProjectile2.GetComponent<EnemyProjectile>().knockback = enemy.specialKnockback;
                    projectileLaunched2 = true;
                }
